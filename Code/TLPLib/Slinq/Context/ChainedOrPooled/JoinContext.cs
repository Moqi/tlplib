...
namespace Smooth.Slinq.Context {
	...
	public struct JoinContext<U, K, T2, T, C> {
		...
		public static Slinq<U, JoinContext<U, K, T2, T, C>> Join(Lookup<K, T2> lookup, Slinq<T, C> outer, Fn<T, K> outerSelector, Fn<T, T2, U> resultSelector, bool release) {
		...
		private readonly Fn<T, K> outerSelector;
		private readonly Fn<T, T2, U> resultSelector;
		...
		private JoinContext(Lookup<K, T2> lookup, Slinq<T, C> outer, Fn<T, K> outerSelector, Fn<T, T2, U> resultSelector, bool release) {
			...
			this.inner = chained.current.isSome && lookup.dictionary.TryGetValue(outerSelector(chained.current.get), out iht) ? iht.head : null;
		...
		private static void Skip(ref JoinContext<U, K, T2, T, C> context, out Option<U> next) {
			...
			if (context.inner == null && context.chained.current.isSome) {
				...
				while (context.chained.current.isSome) {
					context.inner = context.lookup.GetValues(context.outerSelector(context.chained.current.get)).head;
			...
			if (context.chained.current.isSome) {
				next = new Option<U>(context.resultSelector(context.chained.current.get, context.inner.value));
			} else {
				next = new Option<U>();
				context.bd.Release();
				if (context.release) {
		...
		private static void Remove(ref JoinContext<U, K, T2, T, C> context, out Option<U> next) {
			...
			context.inner = context.chained.current.isSome ?
				context.lookup.GetValues(context.outerSelector(context.chained.current.get)).head :
					null;
	...
	public struct JoinContext<U, K, T2, T, C, P> {
		...
		public static Slinq<U, JoinContext<U, K, T2, T, C, P>> Join(Lookup<K, T2> lookup, Slinq<T, C> outer, Fn<T, P, K> outerSelector, Fn<T, T2, P, U> resultSelector, P parameter, bool release) {
		...
		private bool needsMove;
		private readonly Lookup<K, T2> lookup;
		private readonly Fn<T, P, K> outerSelector;
		private readonly Fn<T, T2, P, U> resultSelector;
		...
		private JoinContext(Lookup<K, T2> lookup, Slinq<T, C> outer, Fn<T, P, K> outerSelector, Fn<T, T2, P, U> resultSelector, P parameter, bool release) {
			this.needsMove = false;
			this.lookup = lookup;
			this.outerSelector = outerSelector;
			this.resultSelector = resultSelector;
			...
			LinkedHeadTail<T2> iht;
			this.inner = chained.current.isSome && lookup.dictionary.TryGetValue(outerSelector(chained.current.get, parameter), out iht) ? iht.head : null;
		...
		private static void Skip(ref JoinContext<U, K, T2, T, C, P> context, out Option<U> next) {
			...
			if (context.inner == null && context.chained.current.isSome) {
				context.chained.skip(ref context.chained.context, out context.chained.current);
				while (context.chained.current.isSome) {
					context.inner = context.lookup.GetValues(context.outerSelector(context.chained.current.get, context.parameter)).head;
					if (context.inner == null) {
						context.chained.skip(ref context.chained.context, out context.chained.current);
					} else {
						break;
			...
			if (context.chained.current.isSome) {
				next = new Option<U>(context.resultSelector(context.chained.current.get, context.inner.value, context.parameter));
			} else {
				next = new Option<U>();
				context.bd.Release();
				if (context.release) {
		...
		private static void Remove(ref JoinContext<U, K, T2, T, C, P> context, out Option<U> next) {
			...
			context.inner = context.chained.current.isSome ?
				context.lookup.GetValues(context.outerSelector(context.chained.current.get, context.parameter)).head :
					null;
