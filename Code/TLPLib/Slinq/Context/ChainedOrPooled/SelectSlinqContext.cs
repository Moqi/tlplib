...
namespace Smooth.Slinq.Context {
	...
	public struct SelectSlinqContext<U, UC, T, C> {
		...
		public static Slinq<U, SelectSlinqContext<U, UC, T, C>> SelectMany(Slinq<T, C> slinq, Fn<T, Slinq<U, UC>> selector) {
		...
		private readonly Fn<T, Slinq<U, UC>> selector;
		
		#pragma warning disable 0414
		private BacktrackDetector bd;
		#pragma warning restore 0414

		private SelectSlinqContext(Slinq<T, C> chained, Fn<T, Slinq<U, UC>> selector) {
			this.needsMove = false;
			this.chained = chained;
			this.selected = chained.current.isSome ? selector(chained.current.get) : new Slinq<U, UC>();
		...
		private static void Skip(ref SelectSlinqContext<U, UC, T, C> context, out Option<U> next) {
			...
			if (!context.selected.current.isSome && context.chained.current.isSome) {
				...
				while (context.chained.current.isSome) {
					context.selected = context.selector(context.chained.current.get);
	...
	public struct SelectSlinqContext<U, UC, T, C, P> {
		...
		public static Slinq<U, SelectSlinqContext<U, UC, T, C, P>> SelectMany(Slinq<T, C> slinq, Fn<T, P, Slinq<U, UC>> selector, P parameter) {
		...
		private Slinq<T, C> chained;
		private Slinq<U, UC> selected;
		private readonly Fn<T, P, Slinq<U, UC>> selector;
		...
		private SelectSlinqContext(Slinq<T, C> chained, Fn<T, P, Slinq<U, UC>> selector, P parameter) {
			this.needsMove = false;
			this.chained = chained;
			this.selected = chained.current.isSome ? selector(chained.current.get, parameter) : new Slinq<U, UC>();
			this.selector = selector;
		...
		private static void Skip(ref SelectSlinqContext<U, UC, T, C, P> context, out Option<U> next) {
			...
			if (!context.selected.current.isSome && context.chained.current.isSome) {
				context.chained.skip(ref context.chained.context, out context.chained.current);
				while (context.chained.current.isSome) {
					context.selected = context.selector(context.chained.current.get, context.parameter);
					if (context.selected.current.isSome) {
						break;
					} else {
						context.chained.skip(ref context.chained.context, out context.chained.current);
