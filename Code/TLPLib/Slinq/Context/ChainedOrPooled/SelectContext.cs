...
namespace Smooth.Slinq.Context {
	...
	public struct SelectContext<U, T, C> {
		...
		public static Slinq<U, SelectContext<U, T, C>> Select(Slinq<T, C> slinq, Fn<T, U> selector) {
		...
		private readonly Fn<T, U> selector;
		
		#pragma warning disable 0414
		private BacktrackDetector bd;
		#pragma warning restore 0414

		private SelectContext(Slinq<T, C> chained, Fn<T, U> selector) {
		...
		private static void Skip(ref SelectContext<U, T, C> context, out Option<U> next) {
			...
			next = context.chained.current.isSome ? new Option<U>(context.selector(context.chained.current.get)): new Option<U>();
	...
	public struct SelectContext<U, T, C, P> {
		...
		public static Slinq<U, SelectContext<U, T, C, P>> Select(Slinq<T, C> slinq, Fn<T, P, U> selector, P parameter) {
		...
		private bool needsMove;
		private Slinq<T, C> chained;
		private readonly Fn<T, P, U> selector;
		...
		private SelectContext(Slinq<T, C> chained, Fn<T, P, U> selector, P parameter) {
			this.needsMove = false;
			this.chained = chained;
			this.selector = selector;
		...
		private static void Skip(ref SelectContext<U, T, C, P> context, out Option<U> next) {
			...
			next = context.chained.current.isSome ? new Option<U>(context.selector(context.chained.current.get, context.parameter)): new Option<U>();
