...
namespace Smooth.Slinq.Context {
	...
	public struct SelectOptionContext<U, T, C> {
		...
		public static Slinq<U, SelectOptionContext<U, T, C>> SelectMany(Slinq<T, C> slinq, Fn<T, Option<U>> selector) {
		...
		private readonly Fn<T, Option<U>> selector;
		
		#pragma warning disable 0414
		private BacktrackDetector bd;
		#pragma warning restore 0414

		private SelectOptionContext(Slinq<T, C> chained, Fn<T, Option<U>> selector) {
		...
		private static void Skip(ref SelectOptionContext<U, T, C> context, out Option<U> next) {
			...
			if (context.chained.current.isSome) {
				var o = context.selector(context.chained.current.get);
				while (!o.isSome) {
					context.chained.skip(ref context.chained.context, out context.chained.current);
					if (context.chained.current.isSome) {
						o = context.selector(context.chained.current.get);
					} else {
						break;
					}
				}
	...
	public struct SelectOptionContext<U, T, C, P> {
		...
		public static Slinq<U, SelectOptionContext<U, T, C, P>> SelectMany(Slinq<T, C> slinq, Fn<T, P, Option<U>> selector, P parameter) {
		...
		private bool needsMove;
		private Slinq<T, C> chained;
		private readonly Fn<T, P, Option<U>> selector;
		...
		private SelectOptionContext(Slinq<T, C> chained, Fn<T, P, Option<U>> selector, P parameter) {
			this.needsMove = false;
			this.chained = chained;
			this.selector = selector;
		...
		private static void Skip(ref SelectOptionContext<U, T, C, P> context, out Option<U> next) {
			...
			if (context.chained.current.isSome) {
				var o = context.selector(context.chained.current.get, context.parameter);
				while (!o.isSome) {
					context.chained.skip(ref context.chained.context, out context.chained.current);
					if (context.chained.current.isSome) {
						o = context.selector(context.chained.current.get, context.parameter);
					} else {
						break;
					}
				}
