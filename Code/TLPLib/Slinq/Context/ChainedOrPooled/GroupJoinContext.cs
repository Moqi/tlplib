...
namespace Smooth.Slinq.Context {
	...
	public struct GroupJoinContext<U, K, T2, T, C> {
		...
		public static Slinq<U, GroupJoinContext<U, K, T2, T, C>> GroupJoin(Lookup<K, T2> lookup, Slinq<T, C> outer, Fn<T, K> outerSelector, Fn<T, Slinq<T2, LinkedContext<T2>>, U> resultSelector, bool release) {
		...
		private readonly Fn<T, Slinq<T2, LinkedContext<T2>>, U> resultSelector;
		...
		private GroupJoinContext(Lookup<K, T2> lookup, Slinq<T, C> outer, Fn<T, K> outerSelector, Fn<T, Slinq<T2, LinkedContext<T2>>, U> resultSelector, bool release) {
		...
		private static void Skip(ref GroupJoinContext<U, K, T2, T, C> context, out Option<U> next) {
			...
			if (context.chained.current.isSome) {
				...
				next = new Option<U>(context.resultSelector(
					context.chained.current.get,
					context.lookup.GetValues(context.outerSelector(context.chained.current.get)).SlinqAndKeep(bd)));
				bd.TryReleaseShared();
			} else {
				next = new Option<U>();
				context.bd.Release();
	...
	public struct GroupJoinContext<U, K, T2, T, C, P> {
		...
		public static Slinq<U, GroupJoinContext<U, K, T2, T, C, P>> GroupJoin(Lookup<K, T2> lookup, Slinq<T, C> outer, Fn<T, P, K> outerSelector, Fn<T, Slinq<T2, LinkedContext<T2>>, P, U> resultSelector, P parameter, bool release) {
		...
		private readonly Fn<T, Slinq<T2, LinkedContext<T2>>, P, U> resultSelector;
		...
		private GroupJoinContext(Lookup<K, T2> lookup, Slinq<T, C> outer, Fn<T, P, K> outerSelector, Fn<T, Slinq<T2, LinkedContext<T2>>, P, U> resultSelector, P parameter, bool release) {
			this.needsMove = false;
			this.lookup = lookup;
			this.outerSelector = outerSelector;
			this.resultSelector = resultSelector;
		...
		private static void Skip(ref GroupJoinContext<U, K, T2, T, C, P> context, out Option<U> next) {
			...
			if (context.chained.current.isSome) {
				...
				next = new Option<U>(context.resultSelector(
					context.chained.current.get,
					context.lookup.GetValues(context.outerSelector(context.chained.current.get, context.parameter)).SlinqAndKeep(bd),
					context.parameter));
				bd.TryReleaseShared();
			} else {
				next = new Option<U>();
