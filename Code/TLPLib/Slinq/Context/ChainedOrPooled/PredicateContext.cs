...
namespace Smooth.Slinq.Context {
	...
	public struct PredicateContext<T, C> {
		...
		public static Slinq<T, PredicateContext<T, C>> TakeWhile(Slinq<T, C> slinq, Fn<T, bool> predicate) {
		...
		public static Slinq<T, PredicateContext<T, C>> Where(Slinq<T, C> slinq, Fn<T, bool> predicate) {
			return new Slinq<T, PredicateContext<T, C>>(
				whereSkip,
				whereRemove,
				dispose,
		...
		private readonly Fn<T, bool> predicate;
		
		#pragma warning disable 0414
		private BacktrackDetector bd;
		#pragma warning restore 0414

		private PredicateContext(Slinq<T, C> chained, Fn<T, bool> predicate) {
		...
		private static void TakeWhileSkip(ref PredicateContext<T, C> context, out Option<T> next) {
			...
			if (!context.chained.current.isSome) {
				...
				context.bd.Release();
			} else if (context.predicate(context.chained.current.get)) {
				next = context.chained.current;
			} else {
				next = new Option<T>();
				context.bd.Release();
		...
		private static void WhereSkip(ref PredicateContext<T, C> context, out Option<T> next) {
			...
			while (context.chained.current.isSome && !context.predicate(context.chained.current.get)) {
				context.chained.skip(ref context.chained.context, out context.chained.current);
			}
	...
	public struct PredicateContext<T, C, P> {
		...
		public static Slinq<T, PredicateContext<T, C, P>> TakeWhile(Slinq<T, C> slinq, Fn<T, P, bool> predicate, P parameter) {
		...
		public static Slinq<T, PredicateContext<T, C, P>> Where(Slinq<T, C> slinq, Fn<T, P, bool> predicate, P parameter) {
			return new Slinq<T, PredicateContext<T, C, P>>(
				whereSkip,
				whereRemove,
				dispose,
		...
		private bool needsMove;
		private Slinq<T, C> chained;
		private readonly Fn<T, P, bool> predicate;
		...
		private PredicateContext(Slinq<T, C> chained, Fn<T, P, bool> predicate, P parameter) {
			this.needsMove = false;
			this.chained = chained;
			this.predicate = predicate;
		...
		private static void TakeWhileSkip(ref PredicateContext<T, C, P> context, out Option<T> next) {
			...
			if (!context.chained.current.isSome) {
				next = context.chained.current;
				context.bd.Release();
			} else if (context.predicate(context.chained.current.get, context.parameter)) {
				next = context.chained.current;
			} else {
				next = new Option<T>();
				context.bd.Release();
		...
		private static void WhereSkip(ref PredicateContext<T, C, P> context, out Option<T> next) {
			...
			while (context.chained.current.isSome && !context.predicate(context.chained.current.get, context.parameter)) {
				context.chained.skip(ref context.chained.context, out context.chained.current);
			}
