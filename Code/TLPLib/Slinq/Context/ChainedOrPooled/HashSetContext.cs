...
namespace Smooth.Slinq.Context {
	...
	public struct HashSetContext<T, C> {
		...
		private static void DistinctSkip(ref HashSetContext<T, C> context, out Option<T> next) {
			...
			while (context.chained.current.isSome && !context.hashSet.value.Add(context.chained.current.get)) {
				context.chained.skip(ref context.chained.context, out context.chained.current);
			}
		...
		private static void ExceptSkip(ref HashSetContext<T, C> context, out Option<T> next) {
			...
			while (context.chained.current.isSome && context.hashSet.value.Contains(context.chained.current.get)) {
				context.chained.skip(ref context.chained.context, out context.chained.current);
			}
		...
		private static void IntersectSkip(ref HashSetContext<T, C> context, out Option<T> next) {
			...
			while (context.chained.current.isSome && !context.hashSet.value.Remove(context.chained.current.get)) {
				context.chained.skip(ref context.chained.context, out context.chained.current);
			}
	...
	public struct HashSetContext<K, T, C> {
		...
		public static Slinq<T, HashSetContext<K, T, C>> Distinct(Slinq<T, C> slinq, Fn<T, K> selector, Disposable<HashSet<K>> hashSet, bool release) {
		...
		public static Slinq<T, HashSetContext<K, T, C>> Except(Slinq<T, C> slinq, Fn<T, K> selector, Disposable<HashSet<K>> hashSet, bool release) {
			return new Slinq<T, HashSetContext<K, T, C>>(
				exceptSkip,
				exceptRemove,
				dispose,
		...
		public static Slinq<T, HashSetContext<K, T, C>> Intersect(Slinq<T, C> slinq, Fn<T, K> selector, Disposable<HashSet<K>> hashSet, bool release) {
			return new Slinq<T, HashSetContext<K, T, C>>(
		...
		private bool needsMove;
		private Slinq<T, C> chained;
		private readonly Fn<T, K> selector;
		...
		private HashSetContext(Slinq<T, C> chained, Fn<T, K> selector, Disposable<HashSet<K>> hashSet, bool release) {
			this.needsMove = false;
			this.chained = chained;
		...
		private static void DistinctSkip(ref HashSetContext<K, T, C> context, out Option<T> next) {
			...
			while (context.chained.current.isSome && !context.hashSet.value.Add(context.selector(context.chained.current.get))) {
				context.chained.skip(ref context.chained.context, out context.chained.current);
			}
		...
		private static void ExceptSkip(ref HashSetContext<K, T, C> context, out Option<T> next) {
			...
			while (context.chained.current.isSome && context.hashSet.value.Contains(context.selector(context.chained.current.get))) {
				context.chained.skip(ref context.chained.context, out context.chained.current);
			}
		...
		private static void IntersectSkip(ref HashSetContext<K, T, C> context, out Option<T> next) {
			...
			while (context.chained.current.isSome && !context.hashSet.value.Remove(context.selector(context.chained.current.get))) {
				context.chained.skip(ref context.chained.context, out context.chained.current);
			}
	...
	public struct HashSetContext<K, T, C, P> {
		...
		public static Slinq<T, HashSetContext<K, T, C, P>> Distinct(Slinq<T, C> slinq, Fn<T, P, K> selector, P parameter, Disposable<HashSet<K>> hashSet, bool release) {
		...
		public static Slinq<T, HashSetContext<K, T, C, P>> Except(Slinq<T, C> slinq, Fn<T, P, K> selector, P parameter, Disposable<HashSet<K>> hashSet, bool release) {
			return new Slinq<T, HashSetContext<K, T, C, P>>(
				exceptSkip,
				exceptRemove,
				dispose,
		...
		public static Slinq<T, HashSetContext<K, T, C, P>> Intersect(Slinq<T, C> slinq, Fn<T, P, K> selector, P parameter, Disposable<HashSet<K>> hashSet,  bool release) {
			return new Slinq<T, HashSetContext<K, T, C, P>>(
				intersectSkip,
				intersectRemove,
				dispose,
		...
		private bool needsMove;
		private Slinq<T, C> chained;
		private readonly Fn<T, P, K> selector;
		...
		private HashSetContext(Slinq<T, C> chained, Fn<T, P, K> selector, P parameter, Disposable<HashSet<K>> hashSet, bool release) {
			this.needsMove = false;
			this.chained = chained;
			this.selector = selector;
		...
		private static void DistinctSkip(ref HashSetContext<K, T, C, P> context, out Option<T> next) {
			...
			while (context.chained.current.isSome && !context.hashSet.value.Add(context.selector(context.chained.current.get, context.parameter))) {
				context.chained.skip(ref context.chained.context, out context.chained.current);
			}
		...
		private static void ExceptSkip(ref HashSetContext<K, T, C, P> context, out Option<T> next) {
			...
			while (context.chained.current.isSome && context.hashSet.value.Contains(context.selector(context.chained.current.get, context.parameter))) {
				context.chained.skip(ref context.chained.context, out context.chained.current);
			}
		...
		private static void IntersectSkip(ref HashSetContext<K, T, C, P> context, out Option<T> next) {
			...
			while (context.chained.current.isSome && !context.hashSet.value.Remove(context.selector(context.chained.current.get, context.parameter))) {
				context.chained.skip(ref context.chained.context, out context.chained.current);
			}
