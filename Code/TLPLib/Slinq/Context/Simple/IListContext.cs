...
namespace Smooth.Slinq.Context {
	public struct IListContext<T> {
		...
		public static Slinq<Tpl<T, int>, IListContext<T>> SlinqWithIndex(IList<T> list, int startIndex, int step) {
			return new Slinq<Tpl<T, int>, IListContext<T>>(
				skipWithIndex,
				removeWithIndex,
				disposeWithIndex,
				new IListContext<T>(list, startIndex, step));
		...
		private static readonly Mutator<Tpl<T, int>, IListContext<T>> skipWithIndex = SkipWithIndex;
		private static readonly Mutator<Tpl<T, int>, IListContext<T>> removeWithIndex = RemoveWithIndex;
		private static readonly Mutator<Tpl<T, int>, IListContext<T>> disposeWithIndex = DisposeWithIndex;

		private static void SkipWithIndex(ref IListContext<T> context, out Option<Tpl<T, int>> next) {
			context.bd.DetectBacktrack();
			
			var index = context.index + context.step;
			
			if (0 <= index && index < context.size) {
				next = new Option<Tpl<T, int>>(new Tpl<T, int>(context.list[index], index));
				context.index = index;
			} else {
				next = new Option<Tpl<T, int>>();
				context.bd.Release();
			}
		}
		
		private static void RemoveWithIndex(ref IListContext<T> context, out Option<Tpl<T, int>> next) {
			context.bd.DetectBacktrack();
		...
		private static void DisposeWithIndex(ref IListContext<T> context, out Option<Tpl<T, int>> next) {
			next = new Option<Tpl<T, int>>();
			context.bd.Release();
		}
