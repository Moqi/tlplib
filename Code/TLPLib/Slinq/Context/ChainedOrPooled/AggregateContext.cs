...
namespace Smooth.Slinq.Context {
	...
	public struct AggregateContext<U, T, C> {
		...
		public static Slinq<U, AggregateContext<U, T, C>> AggregateRunning(Slinq<T, C> slinq, U seed, Fn<U, T, U> selector) {
		...
		private U acc;
		private readonly Fn<U, T, U> selector;

		#pragma warning disable 0414
		private BacktrackDetector bd;
		#pragma warning restore 0414

		private AggregateContext(Slinq<T, C> chained, U seed, Fn<U, T, U> selector) {
		...
		private static void Skip(ref AggregateContext<U, T, C> context, out Option<U> next) {
			...
			if (context.chained.current.isSome) {
				context.acc = context.selector(context.acc, context.chained.current.get);
	...
	public struct AggregateContext<U, T, C, P> {
		...
		public static Slinq<U, AggregateContext<U, T, C, P>> AggregateRunning(Slinq<T, C> slinq, U seed, Fn<U, T, P, U> selector, P parameter) {
		...
		private Slinq<T, C> chained;
		private U acc;
		private readonly Fn<U, T, P, U> selector;
		...
		private AggregateContext(Slinq<T, C> chained, U seed, Fn<U, T, P, U> selector, P parameter) {
			this.needsMove = false;
			this.chained = chained;
			this.acc = seed;
			this.selector = selector;
		...
		private static void Skip(ref AggregateContext<U, T, C, P> context, out Option<U> next) {
			...
			if (context.chained.current.isSome) {
				context.acc = context.selector(context.acc, context.chained.current.get, context.parameter);
				next = new Option<U>(context.acc);
			} else {
				next = new Option<U>();
				context.bd.Release();
